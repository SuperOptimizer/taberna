 ...
 *
 *   wind_tv ARC OUT lod z0 y0 x0 dz dy dx priorvol priorlod [lambda=0.3] [niter=250] [zc=mid] [gmin=0.03]
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc,char**argv){
  if(argc<12){ fprintf(stderr,"usage: %s ARC OUT lod z0 y0 x0 dz dy dx priorvol priorlod [lambda=0.5] [niter=250] [zc=mid] [gmin=0.03]\n",argv[0]); return 2; }
  ...
  for(int it=0;it<niter;it++){
    ...
    for(int z=0;z<dz;z++)for(int y=0;y<dy;y++)for(int x=0;x<dx;x++){ size_t p=((size_t)z*dy+y)*dx+x;
      ...
      ub[p]=(f32)(2.0*un-u[p]); u[p]=(f32)un; }
  }
