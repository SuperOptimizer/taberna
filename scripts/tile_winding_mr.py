...
Usage: tile_winding_mr.py ARC FINELOD Z0 Y0 X0 DZ TS NY NX STEP COARSELOD OUTDIR
  All of Z0/Y0/X0/DZ/TS/STEP are in the FINELOD grid (same convention as sheet_sep3d).
  Tiles: NY x NX grid of TS-square crops (single z-band of DZ) stepping STEP in y and x.
...
def main():
    arc, flod, z0, y0, x0, dz, ts, ny, nx, step, clod, outdir = sys.argv[1:13]
    flod, z0, y0, x0, dz, ts, ny, nx, step, clod = map(int, (flod, z0, y0, x0, dz, ts, ny, nx, step, clod))
    os.makedirs(outdir, exist_ok=True)
    # union bbox in FINELOD coords
    uy1, ux1 = y0 + (ny-1)*step + ts, x0 + (nx-1)*step + ts
    pitch_f = 20                                  # native-L2 pitch; scaled per-LOD below
    # --- 1) ONE coarse global solve over the union, at COARSELOD ---
    s = 1 << (clod - flod)                         # fine->coarse downsample factor
    cz0, cy0, cx0 = z0 // s, y0 // s, x0 // s
    cdz, cdy, cdx = max(1, dz // s), (uy1 - y0) // s, (ux1 - x0) // s
    cg = f"{outdir}/coarse"
    print(f"coarse solve @ LOD{clod}: z{cz0} y{cy0} x{cx0} + {cdz}x{cdy}x{cdx}")
    run(arc, clod, cz0, cy0, cx0, cdz, cdy, cdx, cg, max(4, pitch_f // s))
    cgvol = f"{cg}_vol.f32"
    # --- 2) fine tiles, each anchored to the coarse field ---
    tiles = {}
    for iy in range(ny):
        for ix in range(nx):
            ty, tx = y0 + iy*step, x0 + ix*step
            out = f"{outdir}/t_{iy}_{ix}"
            run(arc, flod, z0, ty, tx, dz, ts, ts, out, pitch_f, prior=cgvol, plod=clod)
            tiles[(iy, ix)] = load(f"{out}_vol.f32")
            print(f"tile {iy},{ix} origin {tiles[(iy,ix)][1]}")
    # --- 3) seam agreement (DIRECT, no integer offset) ---
    seams = []
    for (iy, ix), (v0, o0) in tiles.items():
        for diy, dix in ((0, 1), (1, 0)):
            nb = (iy+diy, ix+dix)
            if nb not in tiles:
                continue
            v1, o1 = tiles[nb]
            ...
            if m.sum() < 100:
                continue
            d = a[m] - b[m]
            seams.append((f"{iy},{ix}->{nb[0]},{nb[1]}", float(np.median(d)),
                          float(np.mean(np.abs(d - np.median(d)) < 0.25)),
                          float((d - np.median(d)).std()), int(m.sum())))
    ...
    for v, o in tiles.values():
        fin = np.isfinite(v); sy, sx = o[1]-Y0, o[2]-X0
        acc[:, sy:sy+v.shape[1], sx:sx+v.shape[2]] += np.where(fin, v, 0)
